...
class TestConstantPredictorFlag:
    """``use_constant_predictor`` overrides the side decision to fade the
    market mid (bet YES if mid<0.5, NO otherwise) while leaving the
    selection gate (|p_up - mid| > threshold) untouched. The stored
    ``edge_at_decision`` reflects the *decision* edge (0.5 - mid), and
    ``estimator_p_up`` keeps the legacy Phi(d2) value for post-hoc
    comparison."""

    def _setup(self, p_up_target: Decimal, bid_ask):
        repo = _FakeRepo()
        # Phi(d2) is computed inside evaluate; we don't directly inject p_up.
        # Instead we choose a spot/source combo that lands p_up roughly where
        # we want. To keep this test independent of the lognormal closed form,
        # we just pick large/small spots so p_up sits clearly above or below
        # the market mid, and assert on the *side* of the trade.
        return repo, _FakeBook(bid_ask)

    def test_default_off_uses_phi_d2_side(self):
        """Sanity check: with the flag off and Phi(d2) p_up > mid, side is YES.
        This is the existing behavior and shouldn't change."""
        repo, book = self._setup(Decimal("0.7"), (Decimal("0.30"), Decimal("0.32")))
    ...
    def test_constant_predictor_flips_side_to_fade_mid(self):
        """With the flag on, mid = (0.30+0.32)/2 = 0.31 < 0.5, so the
        constant-predictor view bets YES regardless of Phi(d2)."""
        repo, book = self._setup(Decimal("0.7"), (Decimal("0.30"), Decimal("0.32")))
        sources = {"BTC": _FakeSrc(Decimal("60000"))}
        d = QuantDecider(
            sources=sources, book=book, repo=repo,
            position_size_usd=Decimal("5"),
            use_constant_predictor=True,
        )
        d.evaluate(_row())
        assert len(repo.inserted) == 1
        # mid < 0.5, so fade-the-mid says YES.
        assert repo.inserted[0].side == "YES"
        # edge_at_decision must reflect the decision edge (0.5 - mid),
        # not the legacy Phi(d2) edge.
        edge = float(repo.inserted[0].edge_at_decision)
        assert abs(edge - (0.5 - 0.31)) < 1e-6

    def test_constant_predictor_high_mid_picks_no(self):
        """mid above 0.5 -> fade-the-mid bets NO, even when Phi(d2) might
        also have leaned NO (here the bid/ask is set so mid=0.71)."""
        repo, book = self._setup(Decimal("0.3"), (Decimal("0.70"), Decimal("0.72")))
        sources = {"BTC": _FakeSrc(Decimal("60000"))}
        d = QuantDecider(
            sources=sources, book=book, repo=repo,
            position_size_usd=Decimal("5"),
        ...
        d.evaluate(_row())
        assert len(repo.inserted) == 1
        assert repo.inserted[0].side == "NO"

    def test_hot_reload_flips_subsequent_trades(self):
        """A running decider must be able to flip the policy via
        update_thresholds without restart, mirroring the SIGHUP path."""
        repo, book = self._setup(Decimal("0.7"), (Decimal("0.30"), Decimal("0.32")))
        sources = {"BTC": _FakeSrc(Decimal("60000"))}
        d = QuantDecider(
            sources=sources, book=book, repo=repo,
        ...
        # First trade under default policy: Phi(d2) view (likely YES given
        # spot sits above start_spot/=spot fallback, mid=0.31).
        d.evaluate(_row())
        first_side = repo.inserted[-1].side
        # Flip the flag on; new market evaluated next.
        d.update_thresholds(use_constant_predictor=True)
        # Use a fresh row with different id so the dedup gate doesn't fire.
        row2 = _row()
        row2["id"] = "m2"
        d.evaluate(row2)
        assert len(repo.inserted) == 2
        # mid is still 0.31, so under constant-predictor side is YES.
        assert repo.inserted[-1].side == "YES"
        # Under the flag, edge_at_decision is the constant-predictor edge
        # (0.5 - mid). With mid = 0.31, that is exactly 0.19.
        second_edge = float(repo.inserted[1].edge_at_decision)
        assert abs(second_edge - 0.19) < 1e-6
        _ = first_side  # silence linter
